...
def rbo_page_rank_sm():
    ...
    sm = []
    try:
        cursor.execute('select curr_id, sum(counts) as counts_sum, curr_title from clickstream_derived where link_type_derived=%s group by curr_id order by counts_sum desc limit 10000;', ("internal",))
        result = cursor.fetchall()
        for row in result:
            record = {}
            record['curr_id']= row[0]
    ...
    se = []
    try:
        cursor.execute('select id, sum(counts) as counts_sum from clickstream_derived where link_type_derived=%s group by curr_id order by counts_sum desc limit 10000;', ("entry-se",))
        result = cursor.fetchall()
        for row in result:
            record = {}
            record['curr_id']= row[0]
